...
<div id="tab-music" class="tab" style="display:none;">
    ...
    // Ключ момента входа в бой долины — для 5-минутного таймера
    const UNDYING_FIGHT_START_KEY = 'fadd_undying_fight_start';
    const UNDYING_MAX_FIGHT_MS    = 5 * 60 * 1000; // 5 минут
    ...
    function runUndying(force = false) {
        ...
        if (attackBtn) {
            // Запоминаем момент первой атаки (начало боя) для 5-мин таймера
            if (!localStorage.getItem(UNDYING_FIGHT_START_KEY)) {
                localStorage.setItem(UNDYING_FIGHT_START_KEY, now.toString());
            }
            // Проверяем 5-минутный таймер
            const fightStart = parseInt(localStorage.getItem(UNDYING_FIGHT_START_KEY) || '0', 10);
            if (now - fightStart >= UNDYING_MAX_FIGHT_MS) {
                console.log('[undying] бой идёт >5 мин — завершаем принудительно');
                localStorage.removeItem(UNDYING_FIGHT_START_KEY);
                return false;
            }
        ...
        // ── Если мы на странице /undying/hit/ но кнопки нет —
        //    бой завершился (противник убит или время вышло)
        if (url.includes('/undying/hit/')) {
            const fightStart = parseInt(localStorage.getItem(UNDYING_FIGHT_START_KEY) || '0', 10);
            // Если уже были в бою (есть start) — значит бой кончился, выходим
            if (fightStart > 0) {
                const elapsed = now - fightStart;
                // Даём 5 сек на "может страница просто не обновилась"
                const lastRef = parseInt(localStorage.getItem(UNDYING_REF_KEY) || '0', 10);
                if (now - lastRef < 5000) return true; // ждём обновления
                // Обновляем один раз
                if (elapsed < 8000) {
                    localStorage.setItem(UNDYING_REF_KEY, now.toString());
                    console.log('[undying] кнопки нет — обновляем для проверки');
                    window.location.reload();
                    return true;
                }
                // После 8 сек без кнопки атаки — бой точно окончен
                console.log('[undying] бой завершён (нет атаки на /undying/hit/)');
                localStorage.removeItem(UNDYING_FIGHT_START_KEY);
                return false;
            }
            // fight_start нет — мы попали на /undying/hit/ без боя, обновляем
            const lastRef = parseInt(localStorage.getItem(UNDYING_REF_KEY) || '0', 10);
            if (now - lastRef >= 5000) {
                localStorage.setItem(UNDYING_REF_KEY, now.toString());
                console.log('[undying] на странице боя без start — обновляем');
                window.location.reload();
            }
            return true;
        }

        // ── Если мы на /undying/ и нет кнопки атаки — бой мог завершиться,
        //    сбрасываем таймер и выходим из функции
        if (url.includes('/undying/') && localStorage.getItem(UNDYING_FIGHT_START_KEY)) {
            console.log('[undying] вышли со страницы боя — бой завершён');
            localStorage.removeItem(UNDYING_FIGHT_START_KEY);
            return false;
        }
        ...
        // ── Нет кнопок — ждём начала боя, обновляем каждые 5 сек ─────────────
        const lastRef = parseInt(localStorage.getItem(UNDYING_REF_KEY) || '0', 10);
        if (now - lastRef >= 5000) {
            localStorage.setItem(UNDYING_REF_KEY, now.toString());
